...
Answer: The relationship appears to be linear and positively correlated.
...
Answer: Yes, this proves that the relationship is linear and positively correlated.
...
ggplot(birds, aes(male_mass, egg_mass)) +
  ...
  annotate("text", x = 220, y = 75, label = "Tropicbird")
...
```{r}
mating2 <- filter(birds, mating_system == 2)
```



```{r}
ggplot(mating2, aes(male_mass, egg_mass)) +
  geom_point() +
  scale_x_log10() +
  scale_y_log10() +
  geom_smooth(method = "lm", color = "black", se = FALSE, linetype = "dashed", size = 0.5) +
  geom_point(color = "red", data = parrots) +
  annotate("point", x = 218.7, y = 87.00, color = "red", size = 5) +
  annotate("text", x = 220, y = 75, label = "Tropicbird") +
  ggtitle("Mating System 2")
```
