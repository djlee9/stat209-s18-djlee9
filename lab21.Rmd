...
**Answer**: The distribution is negatively skewed. A bar plot works because we are only measuring the value one variable.

Produce a confidence interval for the mean of your trait.

```{r}
model <- lm_basic(reasoning ~ 1, data = pf)
...
model1 <- lm_basic(reasoning ~ 1 + gender, data = HU)
...
**Answer**: The confidence interval is much wider, this is probably because there are fewer data points and therefore less statistical confidence.
...
**Answer**: There is no statistical significant difference between men and women.
...
```{r}
plus30 <- filter(pf, age >= 30)
between30_49 <- filter(plus30, age <= 49)
between30_49$fourties <- between30_49$age >= 40
```
...
```{r}
model2 <- lm_basic(reasoning ~ 1 + fourties, data = between30_49)
...
**Answer**: There is no statistical significant difference between the two age groups.
...
```{r}
ggplot(pf, aes(country, reasoning)) +
  geom_col()
...
```{r}
model3 <- lm_basic(reasoning ~ 1 + fct_lump(country, n = 5), data = pf)
...
**Answer**: Canada has the highest score for reasoning. India has the lowest.
...
```{r}
model4 <- lm_basic(reasoning ~ 1 + sensitivity, data = pf)
...
**Answer**: Yes, that shows the relationship between sensitivity and reasoning.

Fit a regression model that uses both gender and the baseline score to
predict your trait.

```{r}
model4 <- lm_basic(reasoning ~ 1 + sensitivity + gender, data = pf)
reg_table(model4, level = 0.95)
```

Describe the slope for the gender term in words:

**Answer**: The slope for gender represents the change in the intercept based on the change in the gender.

Add predictions from the previous model to `pf`.

```{r}
pf <- add_prediction(pf, model4)
```
...
```{r}
ggplot(pf, aes(model4_pred))+
  geom_bar(aes(fill = gender))
```

What do the predicted values look like?

**Answer**: The predicted values look very similar to my original plot.
