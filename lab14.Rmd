...
```{r}
ggplot(bird_type, aes(egg_mass_median, male_mass_mean)) +
  geom_point() +
  scale_x_log10() +
  scale_y_log10()
```
...
```{r}
five_plus_birds <- filter(bird_type, n >= 5)
```
...
ggplot(bird_type, aes(egg_mass_median, male_mass_mean)) +
  ...
  scale_x_log10() +
  scale_y_log10() +
  geom_point(color = "red", size = 3, data = five_plus_birds) +
  geom_text_repel(aes(label = type), data = five_plus_birds)
